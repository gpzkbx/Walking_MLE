...
public class VSDirector : MonoBehaviour {
    ...
    public static int trial_index; //0 for statard trial, 1 for comparison trial
    ...
    void Run_trial()
    {
        ...
        if (this.noise_flag)
        {
            ...
            //Debug.Log("noise_index = " + noise_index);
        ...
        if (this.delta > wait_span && stimulus_flag == 0)
        {
            ...
            GameObject InsSphere = Instantiate(this.mysphere) as GameObject;

            spot_index = ns_index / 5;
            float px = spot_candidate[spot_index];
            //Debug.Log("spot_index : " + spot_index);

            trial_index = Random.Range(0, trial_candidate.Count);
            if (trial_candidate[trial_index])
            {
                InsSphere.transform.position = new Vector3(px, stimulus_height, 5);
            }
            else
            {
                InsSphere.transform.position = new Vector3(0, stimulus_height, 5);
            }
